...
| **L3** | ~~The `lint` script is `eslint … --fix`, so running the CI lint command **rewrites 20 source files** with Prettier formatting (`prettier/prettier` is an `error` here). The committed tree is not Prettier-clean. CI does not notice because the rewrite happens before the report, but any developer running `pnpm run lint` gets an unrelated 20-file diff. CI should run `eslint` without `--fix`, and the tree should be formatted once.~~ **Partially fixed on `fix/build-and-lint`:** the tree is now Prettier-clean (commit `7f0d57b`), so `pnpm run lint` no longer rewrites 20 files. Still open: the `lint` script keeps `--fix`, and CI runs that script, so future formatting drift is auto-corrected in the CI container instead of failing the build. Closing this needs a non-fixing lint command for CI — e.g. a `lint:ci` script running `eslint` without `--fix`, used by `bitbucket-pipelines.yml`. |
