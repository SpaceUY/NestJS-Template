...
**Read this first.** Everything below this section is the audit as written on
2026-09-18 (plus the remediation notes added to it since). It describes a tree
that has changed a great deal — `!src/spaceship/` and `!src/user/` are deleted,
`analytics` moved out of `common/observability/` to the top level as
`src/analytics/`, and `!src/queues/queues.module.ts` is gone. This section is
the one place to read what is actually open today; the body is the history of
how each finding got there.

Every finding in both audits was re-verified against the working tree on
2026-09-21 — file opened, grep re-run — not against this document and not
against the module guides.

**Open today — 13 findings plus one accepted debt:** eleven defects (`N5` and
ten `M` rows), `EXT7`'s invisible coupling, `EXT9`'s coverage boundary, and
`H4`.

| ID | What is still open |
|----|--------------------|
| `N5` | Four modules ship `abstract/mocks/` (`cache`, `cloud-storage`, `email`, `push-notification`); `config-provider`, `queues`, `templating`, `common/observability/logger` and `analytics` ship none. |
| `M8` | Disputed tier calls in `scripts/check-module-independence.mjs`'s `TIERS` map — `database` is `infrastructure` while owning the `User` entity, `templates` is `feature` while only `email` consumes it. Narrowed: the consequence this finding recorded (`M4`'s tier violation) is gone, since the graph now has zero violations. |
| `M9` | Checker coverage boundary. Narrowed: `analytics` is its own top-level module now, so `analytics -> common` is a real edge in the graph (10 sites). `common/observability/{logger,telemetry}` still fold into one `common` node, so a `logger`-only dependency still reads as a dependency on all of `common`. |
| `M10` | Directory-shape deviation, `queues`. Narrowed: the bespoke root composition module is deleted. What remains is the split `abstract/producer/` + `abstract/consumer/` layer, which the contract's diagram has no room for and `src/queues/CLAUDE.md` documents as deliberate. |
| `M11` | Directory-shape deviation, `templating`: `abstract/` holds only `template-provider.const.ts` and `template.service.ts`; no `template.interfaces.ts`, no `template.error.ts`, no `template-abstract.module.ts`, and the dynamic module sits at the module root in `src/templating/template.module.ts`. |
| `M12` | Rule 4, `templating`: `src/templating/pug-adapter/pug-adapter.service.ts:14-21` still has no `try`/`catch`, and there is still no module error type to translate into (`M11`). |
| `M13` | Rule 4, `common/observability/logger`: still no `logger.error.ts` in `src/common/observability/logger/abstract/`. |
| `M14` | Rule 2, `push-notification`: `src/push-notification/expo-adapter/expo-adapter.service.ts:24-26` is still the one adapter decorating its config parameter with `@Inject`; every other adapter takes a plain parameter. |
| `M16` | Rule 4, `analytics`: still no error class under `src/analytics/abstract/`, and the PostHog adapter still catches and returns a safe default by documented design. |
| `M17` | Rule 4, `cloud-storage`: `src/cloud-storage/local-adapter/local-adapter.service.ts` still has no `try`/`catch` around `mkdir`/`writeFile` in `uploadFile`, and `deleteFile`/`getFile` still re-throw every non-`ENOENT` error raw. |
| `M18` | Rule 4, `email`: `src/email/aws-ses-adapter/aws-ses-adapter.service.ts` still never imports `EmailError`; its one `catch` (`:81-84`) re-throws the raw `@aws-sdk/client-ses` rejection. |
| `EXT7` | `common`'s ambient-type dependency on the repo-root `@types/express/index.d.ts` is real and unchanged — `src/common/middleware/response.interceptor.ts:54` still reads `user.id`. The *documentation* half (`DOC3`) is closed: `src/common/CLAUDE.md:107-111` and `src/common/README.md:97-99` both name the augmentation and the `typeRoots` entry now. |
| `EXT9` | Still accurate as a permanent caveat: `scripts/check-module-independence.mjs` matches import specifiers, so an ambient `declare namespace` dependency has nothing to match and a green `modularity:check` is not proof a module extracts cleanly. |
| `H4` | `test/app.e2e-spec.ts` is unmodified Nest scaffold needing a live database and Redis. Tracked in the root `CLAUDE.md`, not in either audit table. **Accepted as debt on 2026-09-21** by team decision, with both exits written down: real e2e specs with `postgres`/`redis` services in `bitbucket-pipelines.yml`, or delete the file, `test/jest-e2e.json` and the `test:e2e` script. |

**Known pending, by team decision on 2026-09-21 — not a finding, not closed.**
The extraction measurements are not uniformly current. Only `queues` was
re-measured after the 2026-09-19 discovery that a fresh `@nestjs/cli` scaffold
is ESM while this template is CommonJS, which is what made the earlier probes
measure the wrong thing (`docs/audit/evidence/extract-queues-2026-09-19.txt`,
`docs/audit/evidence/extract-queues-closure-2026-09-19.txt`). `cache` (`EXT1`,
`EXT6`) and `email` (`EXT2`, `EXT5`) date from 2026-09-18, before that
discovery. The other thirteen top-level directories under `src/` have never
been copied into a clean project and compiled at all. Until they are, no
per-module extraction cost outside `queues` should be quoted as measured.

`src/health/`, added 2026-09-21, is the thirteenth and the one exception to the
shape of this gap: it has never been *extracted*, but unlike every other module
here it has been verified *booting* — against the `postgres` and `redis`
containers, with each dependency stopped in turn. See
`src/health/README.md`'s `## Verified against real infrastructure`. That is a
different axis from extraction, and the reason it is worth naming: every
number in this audit is a compile-time number.

**Closed: 51 of the 69 findings across both documents.** From this audit
(37 IDs): `M1`-`M7` and `M15`, `EXT3`/`EXT4`, and all ten `DOC` findings — 20
closed, 12 open, and 5 `EXT` rows that are reference measurements rather than
defects (`EXT1`, `EXT2`, `EXT5`, `EXT6`, `EXT8`). From
`docs/audit/2026-09-11-template-audit.md` (32 IDs): 31 closed, every one
struck through in that file with its own evidence, `N5` the lone exception. The two the
2026-09-18 body is most wrong about are `EXT3`/`EXT4` — re-measured
2026-09-19, `queues` now closes at **two** companion directories,
`src/common/observability/logger/` and `src/config-provider/`, with
`tsc --noEmit` exit 0, not the 11-of-13 closure recorded below.

**Gates.** `pnpm run docs:check` and `pnpm run modularity:check` pass, the
latter against a zero-violation baseline (`docs/audit/module-independence-baseline.json`).
The gate table further down records the 2026-09-18 measurement and is not
re-stated here; the root `CLAUDE.md`'s `## Known template-wide gaps` carries
the current one.

**Per-series count, 2026-09-21.** `B` 3/3 closed · `N` 6/7 closed (`N5` open)
· `D` 5/5 closed · `C` 5/5 closed · `TS` 3/3 closed · `L` 3/3 closed ·
`R` 4/4 closed · `G` 2/2 closed · `M` 8/18 closed (10 open) ·
`EXT` 2/9 closed, 5 are reference data rather than defects, 2 open
(`EXT7`, `EXT9`) · `DOC` 10/10 closed · `H4` open by decision.
...
### Breaks the promise (8 as measured; **0 still open as of 2026-09-21** — the whole bucket is closed)

> **2026-09-21.** All eight are closed. `M2`, `M3` and `M5` closed on
> `feature/queues-decoupling` (2026-09-19); `EXT3`/`EXT4` were re-measured the
> same day and `queues` now closes at two companion directories, not eleven;
> `DOC2`, `DOC5` and `DOC7` closed on `fix/documentation-remediation`. The
> bucket text below is the 2026-09-18 reading.
>
> **2026-09-22.** `src/spaceship/` is back, as the reference domain module, on
> `feature/spaceship-reference-module`. The bucket stays closed and was
> re-measured to say so: 27 edges, no strongly connected component, and
> `queues -> {common, config-provider}` unchanged. What closed `M3`/`M5` was
> `QueueConsumerModule.forFeature` — a domain module binds its own handler, so
> the arc runs `spaceship -> queues` and never back. Deleting the demo module
> was a consequence of that change, not its mechanism, and the rows for `M3`
> and `M5` now say so where they used to cite the module's absence as evidence.
...
### Erodes the promise (19 as measured; **8 still open as of 2026-09-21** — `M9`, `M10`, `M12`, `M13`, `M17`, `M18`, `EXT7`, `EXT9`)

> **2026-09-21.** Eleven of the nineteen are closed: `M1`, `M4`, `M6`, `M7`
> and `M15`, plus all six `DOC` findings in this bucket. `M9` and `M10` are
> narrowed rather than closed — see `## Status as of 2026-09-21`. The bucket
> text below is the 2026-09-18 reading.
...
### Cosmetic (10 as measured; **4 defects still open as of 2026-09-21** — `M8`, `M11`, `M14`, `M16`; `DOC6` fixed; the five `EXT` rows are reference data, not defects)

> **2026-09-21.** `DOC6` is closed. `M8` is narrowed (its stated consequence,
> `M4`'s tier violation, no longer exists — the graph has zero violations).
> `M11`, `M14` and `M16` are unchanged. Of the reference-data rows, `EXT1`,
> `EXT6` and `EXT8` were re-verified today; `EXT2` and `EXT5` have not been
> re-measured since 2026-09-18 and predate the ESM-scaffold correction — see
> the "Known pending" note in `## Status as of 2026-09-21`.
...
> **2026-09-21.** `DOC1`-`DOC6` were re-checked against today's tree and all
> six remain correctly closed — including `DOC1`, whose guide was corrected a
> second time when `M15` was fixed: `src/push-notification/CLAUDE.md:70` now
> reads "Failures throw `PushNotificationError`", which matches
> `src/push-notification/expo-adapter/expo-adapter.service.ts:49,75,152`.
> `DOC5`'s target, the root `CLAUDE.md`'s `## Known template-wide gaps`, was
> rewritten again in this same pass and is current.
...
> **2026-09-21.** `DOC7`, `DOC8` and `DOC9` were re-checked and all three
> remain correctly closed. `DOC7`'s subject moved on: `src/queues/README.md`
> still carries its `## Reuse` section, but the extraction cost it describes
> is now the corrected two-companion one, not the 11-of-13 closure, because
> `!src/queues/queues.module.ts` no longer exists.
...
> **Reversed on 2026-09-21.** Leaving that file untouched is what let its
> findings stay unstruck long after they were fixed, and citations from the
> module guides resolve to *that* file, not to this table. The 2026-09-21
> status pass therefore struck each finding in
> `docs/audit/2026-09-11-template-audit.md` itself, with its own evidence.
> **Every verdict in the table below is as of 2026-09-18 and is superseded**
> by that file and by `## Status as of 2026-09-21` at the top of this one:
> today the count is **31 fixed of 32**, the one exception being `N5`, which is
> narrowed but still open. Do not read a "Still open" below as current.
...
**Current tally (status pass, 2026-09-21):** **31 fixed of 32** — `B1`-`B3`,
`N1`-`N4`, `N6`, `N7`, `D1`-`D5`, `C1`-`C5`, `TS1`-`TS3`, `L1`-`L3`,
`R1`-`R4`, `G1`, `G2`. **1 still open:** `N5`, narrowed from "only `cache`
ships mocks" to five modules that still do not. 0 partially fixed — `L3`,
`G1` and `G2` all closed since the tally above, and `N5` is the only
"partially" left, counted as open. 0 superseded. Each verdict was reached by
opening the file or re-running the grep against the working tree, and each is
struck in `docs/audit/2026-09-11-template-audit.md` with that evidence.
...
**Amended 2026-09-21.** That freeze still holds — the body's prose was not
rewritten. What the 2026-09-21 status pass added on top of it: a
`## Status as of 2026-09-21` section at the head of this file, corrected
counts in the three priority-bucket headers, a per-finding status note struck
or appended to every `M`, `EXT`, `DOC` and prior-audit row, and a note over
the conformance matrix and the prior-audit reconciliation table saying both
are superseded. This tail section listed only the `M` series; `EXT3` and
`EXT4` were closed by the same branch's re-measurement on the same day and
are now struck in the Extraction table above.
