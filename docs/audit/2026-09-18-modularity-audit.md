...
| ID | Finding |
|----|---------|
| **EXT1** | `cp -r src/cache <probe>/src/cache && tsc --noEmit` exits **0**, no output (`extract-cache.txt` is empty). `cache` has no relative import that leaves its own directory — `grep -rhn "from '\.\./\.\." src/cache --include='*.ts'` matches only paths that resolve back inside `cache` itself (e.g. `src/cache/redis-adapter/extensions/redis-cache-keys.extension.ts:4`, `../../abstract/cache.error`) — and no source file mentions `common` or `config-provider`. **This contradicts this task's briefed expectation** that `cache` imports from `common` and `config-provider`: the Task 3 evidence itself (`.superpowers/audit-evidence/module-graph.txt`) has no `cache -> common` or `cache -> config-provider` line; every edge touching `cache` is inbound (`(app) -> cache`, `spaceship -> cache`). Verdict: **lifts cleanly**. |
...
| **EXT7** | **Two residual errors survive every closure, neither is a missing module.** Both the `email` closure (EXT5) and the `queues` closure (EXT4) leave the same 2 non-`TS2307` errors even once every intra-repo import resolves. (a) `src/common/middleware/response.interceptor.ts:40` — `TS2339: Property 'id' does not exist on type 'User'` — fails in every probe combination tried here (including EXT4's 11-directory closure, and a variant of it with `auth`+`database` added specifically to test this), but does **not** fail in a full-tree `tsc --noEmit -p tsconfig.json` run from the template root (confirmed: that run's only error is an unrelated `test/app.e2e-spec.ts` supertest typing issue, `TS2349`). `common` never imports anything auth-related, so this is not an edge the Task 3 import graph could ever have found — it is a global ambient-type dependency (the `Express.User` interface) that something elsewhere in a full build supplies, and no probe combination tried here reproduced supplying it. Recorded as real, reproducible, and a class of coupling that only an extraction probe (not a static import graph) can surface, even though this task did not fully identify its source. (b) `src/config-provider/abstract/config-provider-abstract.module.ts:223` — `TS2322`, a `Provider[]` assignability error — is a probe-harness artifact, not a coupling defect: it disappears when the probe's `tsconfig.json` adds `strictNullChecks: true` (confirmed by rerunning EXT5's probe with only that flag added), which is the one strictness flag the template's own root `tsconfig.json` sets and the brief's from-scratch probe config (deliberately) does not. Recorded so a reader does not mistake it for a fourth missing module. |
