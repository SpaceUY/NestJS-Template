...
import { TasksModule } from "@/modules/core/tasks/background/tasks.module";
...
describe("tasksModule", () => {
  describe("forRoot", () => {
    it("should throw an error if no task sequence modules are provided", () => {
      expect(() => TasksModule.forRoot([])).toThrow(
    ...
    it("should create a dynamic module with the correct imports and providers", () => {
      ...
      const tasksModule = TasksModule.forRoot([sequenceModule]);
      ...
      expect(providerTypes).toContain(TasksRepository);
    ...
    it("should set up the sequence registry provider correctly", () => {
      ...
      const tasksModule = TasksModule.forRoot([sequence1, sequence2]);

      // Find the SequenceRegistry provider
      const sequenceRegistryProvider = tasksModule.providers.find(
        provider => typeof provider !== "function" && provider.provide === SequenceRegistry,
      );

      // Check that it's a factory provider with the correct tokens injected
      expect(sequenceRegistryProvider).toBeDefined();
      expect((sequenceRegistryProvider as FactoryProvider<any>).useFactory).toBeDefined();

      // Check that all sequence tokens are injected
      const injectedTokens = (sequenceRegistryProvider as FactoryProvider<any>).inject;
      expect(injectedTokens).toContain(createSequenceDefinitionToken(sequence1Name));
      expect(injectedTokens).toContain(createSequenceDefinitionToken(sequence2Name));
      expect(injectedTokens).toContain(createSequenceErrorHandlerToken(sequence1Name));
      expect(injectedTokens).toContain(createSequenceErrorHandlerToken(sequence2Name));
      expect(injectedTokens).toContain(createSequenceSuccessHandlerToken(sequence1Name));
      expect(injectedTokens).toContain(createSequenceSuccessHandlerToken(sequence2Name));
    });
    ...
    it("should set up the task registry provider correctly", () => {
      ...
      const taskRegistryProvider = tasksModule.providers.find(
        provider => typeof provider !== "function" && provider.provide === TaskRegistry,
      );

      // Check that it's a factory provider with the correct tokens injected
      expect(taskRegistryProvider).toBeDefined();
      expect((taskRegistryProvider as FactoryProvider<any>).useFactory).toBeDefined();

      // Check that sequence definition tokens are injected
      const injectedTokens = (taskRegistryProvider as FactoryProvider<any>).inject;
      expect(injectedTokens).toContain(createSequenceDefinitionToken(sequenceName));
    });
    ...
    it("should handle multiple sequence modules correctly", () => {
      ...
      const tasksModule = TasksModule.forRoot(sequences);
      ...
      for (const name of ["seq-1", "seq-2", "seq-3"]) {
        ...
        expect(injectedTokens).toContain(createSequenceSuccessHandlerToken(name));
      }
    });
  });
});
