...
- **`R4`** — ~~`src/database/migrations/` holds only `.gitkeep`; the template
  ships no baseline migration.~~ **Fixed on `chore/base-migration`:**
  `src/database/migrations/1789989996617-InitialSchema.ts` creates the whole
  current schema, generated by the CLI against an empty database (`T8`).
- **`H6`** — ~~`pnpm run db:migration:run` failed on any fresh database.~~ The
  directory's only migration was `AddAuth0IdToUser`, which begins
  `ALTER TABLE "user"` — a table no migration ever created, because it had only
  ever existed through `DB_SYNCHRONIZE` in local development. A fresh deploy
  therefore crashed in the container entrypoint, which runs
  `pnpm run db:migration:run`. **Fixed on `chore/base-migration`:** that
  migration is deleted and the generated baseline contains what it did — the
  `auth0Id` column, its unique constraint and the `AUTH0` enum value.
